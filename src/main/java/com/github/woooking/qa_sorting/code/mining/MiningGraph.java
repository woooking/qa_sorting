...
import com.github.woooking.qa_sorting.adt.LispList;
import com.github.woooking.qa_sorting.adt.impl.EdgeImpl;
...
//		});
	...
	public static <N, E> List<Graph<N, E>> resultFilter(Collection<Fragment<N, E>> result) {
		...
		List<Set<com.github.woooking.qa_sorting.adt.graph.Edge<N>>> edgeSets = new ArrayList<>();
		result.stream().map(Fragment::toGraph).sorted(Ordering.natural().reverse().onResultOf(Graph::getEdgeCount)).forEach(graph -> {
			...
			Set<com.github.woooking.qa_sorting.adt.graph.Edge<N>> edges = StreamSupport.stream(ite.spliterator(), false).map(EdgeImpl<N>::new).collect(Collectors.toSet());
			boolean add = true;
			for (Set<com.github.woooking.qa_sorting.adt.graph.Edge<N>> edgeSet : edgeSets) {
